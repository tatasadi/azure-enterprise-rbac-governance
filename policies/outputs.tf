output "policy_definitions" {
  ...
  value = {
    ...
    audit_privileged_roles_landingzones = {
      id   = azurerm_policy_definition.audit_privileged_roles_landingzones.id
      name = azurerm_policy_definition.audit_privileged_roles_landingzones.display_name
    }
    audit_privileged_roles_platform = {
      id   = azurerm_policy_definition.audit_privileged_roles_platform.id
      name = azurerm_policy_definition.audit_privileged_roles_platform.display_name
    }
    require_rg_tags = {
...
output "policy_assignments" {
  ...
  value = {
    ...
    audit_privileged_landingzones = {
      id    = azurerm_management_group_policy_assignment.audit_privileged_landingzones.id
      scope = data.azurerm_management_group.landing_zones.id
    }
    audit_privileged_platform = {
      id    = azurerm_management_group_policy_assignment.audit_privileged_platform.id
